    ...
    public class ImageService : BaseService, IImageService
    {
        ...
        public async Task<T> CreateImageAsync<T>(CreateImageDtoBase imageDto, string token)
        {
            return await this.SendAsync<T>(new ApiRequest()
            {
                ...
                Data = imageDto,
                Url = StaticDitels.ProductApiBase + "/api/v1/image",
                AccessToken = token
            });
        }
        ...
        public async Task<T> DeleteImageAsync<T>(int id, string token)
        {
            return await this.SendAsync<T>(new ApiRequest()
            {
                ...
                Url = StaticDitels.ProductApiBase + $"/api/v1/image/{id}",
                AccessToken = token
            });
        }
        ...
        public async Task<T> GetAllImageAsync<T>(string? filter, string? search, string token)
        {
            return await this.SendAsync<T>(new ApiRequest()
            {
                ...
                Url = StaticDitels.ProductApiBase + $"/api/v1/images?filter={filter}&search={search}",
                //AccessToken = token                          
            });
        }
